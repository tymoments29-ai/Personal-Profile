...
interface TagsInputProps {
  value: string; // comma-separated: "linux-basics,ubuntu,ssh"
  ...
  return (
    <div className="flex flex-wrap gap-2 p-2 min-h-[44px] rounded-md border border-border bg-muted focus-within:ring-2 focus-within:ring-ring focus-within:ring-offset-2">
      {tags.map((tag) => (
        <span
          key={tag}
          className="inline-flex items-center gap-1 px-2.5 py-1 rounded-md text-xs font-medium bg-blue-100 text-blue-700 dark:bg-blue-500/15 dark:text-blue-400"
        >
          {tag}
          <button
            type="button"
